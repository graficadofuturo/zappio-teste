...
router.all("/", async (req: any, res) => {
  ...
  try {
    switch (action) {
      ...
      case "collect-offers":
      case "sync-offers": {
        const { collectAutomatedFromML, saveToOfferBank } = await import("../mlService.ts");
        const items = await collectAutomatedFromML("ofertas do dia", "Geral");
        let count = 0;
        if (items.length > 0) {
            count = await saveToOfferBank(items);
        }
        return res.json({ ok: true, totalSaved: count });
      }
    ...
    console.error(`ML_ACTION_ERR_${action}`, error);
    return res.status(500).json({ ok: false, error: error.message });
  }
});

router.get("/auth", async (req, res) => {
  // Aliases for /auth-url
  ...
  const encodedState = encodeURIComponent(Buffer.from(JSON.stringify(stateObj)).toString('base64url'));
