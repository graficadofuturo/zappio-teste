...
export default function PhonePreview({ message, imageUrl, dummyProduct }: PhonePreviewProps) {
  ...
  const formatText = (text: string) => {
    ...
    let replacedText = renderOfferMessage(text, prod);

    return replacedText.split('\n').map((line, i) => {
      // Basic strikethrough detection for preview
      if (line.includes('~')) {
        return (
          <span key={i} className="block min-h-[20px]">
            {line.split(/(~[^~]+~)/g).map((part, j) => {
              if (part.startsWith('~') && part.endsWith('~')) {
                return <span key={j} className="line-through">{part}</span>;
              }
  ...
  } else if (!finalImageUrl && dummyProduct) {
      finalImageUrl = dummyProduct.product_image || dummyProduct.imageUrl || dummyProduct.image || dummyProduct.thumbnail || '';
  }
