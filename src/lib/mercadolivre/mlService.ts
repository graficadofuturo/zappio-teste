...
export async function getMLAuthUrl(origin: string, state: string, redirectUri: string) {
    ...
    return `https://auth.mercadolivre.com.br/authorization?response_type=code&client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&state=${encodeURIComponent(state)}&scope=offline_access`;
}
