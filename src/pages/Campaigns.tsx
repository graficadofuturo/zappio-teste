    ...
    try {
      ...
      for (const target of targetList) {
          ...
          try {
            ...
            const res = await fetchJson('/api/whatsapp/send', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                ...
                image_url: prepData.imageUrl || camp.image_url || ''
              })
            });

            if (!res || (res.success !== true && !res.ok)) {
              throw new Error(res?.error || 'Erro ao enviar mensagem via WhatsApp');
            }

            // Mark as sent in history if it was an auto offer
            if (prepData.productId) {
              await fetchJson('/api/campaigns/mark-sent', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            await addDoc(collection(db, 'campaign_logs'), {
              campaignId: id,
              productId: prepData.productId || null,
      ...
      if (errors.length === targetList.length && targetList.length > 0) {
          throw new Error('Falha total no envio: ' + errors[0]);
      }

      // Restore status based on rules
      let finalStatus = (camp.is_recurring || camp.trigger_type === 'auto') ? 'scheduled' : 'sent';
      ...
      await updateDoc(doc(db, 'campaigns', id), {
        ...
        lastSendError: errors.length > 0 ? `${errors.length} erros no envio` : null,
