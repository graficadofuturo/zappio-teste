...
export default function Campaigns() {
  ...
  useEffect(() => {
    ...
    fetchMarketplaces();
  }, [isCreating, messageMode, auth.currentUser]);
          ...
          {campaignsList.length === 0 ? (
            ...
            <div className="flex flex-wrap items-center gap-2 text-[13px] text-gray-500 mt-1">
              ...
              <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wider ${
                camp.status === 'sent' ? 'bg-green-50 text-green-700 border border-green-200' : 
                camp.status === 'failed' ? 'bg-red-50 text-red-700 border border-red-200' : 
                camp.status === 'paused' ? 'bg-orange-50 text-orange-700 border border-orange-200' : 
                camp.status === 'needs_manual_action' ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : 
                camp.status === 'partially_sent' ? 'bg-purple-50 text-purple-700 border border-purple-200' : 
                'bg-blue-50 text-blue-700 border border-blue-200'
              }`}>
                  {camp.status === 'sending' ? 'Enviando...' : 
                   camp.status === 'queued' ? 'Na Fila' : 
                   camp.status === 'sent' ? 'Enviada' : 
                   camp.status === 'partially_sent' ? 'Parcial' : 
                   camp.status === 'needs_manual_action' ? 'Ação Manual' : 
                   camp.status === 'scheduled' ? 'Ativa' : 
            ...
            {camp.last_run_message && ['failed', 'needs_manual_action', 'partially_sent'].includes(camp.status) && (
                <div className="mt-2 text-[12px] text-red-600 bg-red-50 p-2 rounded-md border border-red-100 break-words">
                  <strong>Alerta:</strong> {camp.last_run_message}
                </div>
            )}
    ...
    <div className="flex-1 flex flex-col h-full bg-gray-50/50">
        ...
        <div>
          ...
          <div className="flex gap-3">
              <button 
                ...
                className="px-5 py-2.5 bg-accent-orange text-white rounded-xl font-semibold text-[13px] flex items-center gap-2 hover:bg-accent-orange/90 transition-colors disabled:opacity-50 shadow-sm"
              >
                {isSendingNow ? (
                  <>
          ...
          >
            {isSavingCampaign ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Salvar Campanha'}
          </button>
        </div>
      </header>
            ...
            {isAiSectionOpen && (
              <div className="p-5 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  ...
                  >
                    {isAiGeneratingCopy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4"/>} 
                    {isAiGeneratingCopy ? 'Processando...' : 'Aplicar com IA'}
                  </button>
                </div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                       ...
                       >
                         ...
                         {!marketplaces.some(m => m.id === 'mercadolivre_global') && (
      ...
      {/* Live Preview Pane */}
        <div className="flex flex-col items-center sticky top-0">
          ...
          />
          {/* Aviso discreto se não converter */}
          {(() => {
            ...
            if (!isGeneratingPreview && prod?.affiliatePending) {
              return (
                <div className="mt-4 flex items-start gap-2 bg-yellow-50 text-yellow-800 p-3 rounded-lg border border-yellow-200 text-xs w-full max-w-[320px]">
