...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', userCredential.user.uid), { is_admin: false, email: userCredential.user.email });
      setSuccess(true);
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-tertiary flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-primary p-8 rounded-xl border border-subtle shadow-sm">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-primary">Crie sua conta</h2>
          <p className="text-secondary text-sm mt-1">Automatize suas vendas hoje</p>
        </div>
        ...
        {success ? (
          <div className="text-center">
            <div className="mb-4 p-3 bg-green-50 border border-green-200 text-green-700 rounded-md text-sm font-medium">
              Conta criada com sucesso! Você já pode acessar o painel.
            </div>
            <Link to="/auth/login" className="text-accent-primary font-medium hover:underline">Ir para o Login</Link>
          </div>
        ) : (
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-primary mb-1">Senha (Mín. 6 caracteres)</label>
              <input
                type="password"
                required
                minLength={6}
        ...
        <div className="mt-6 text-center text-sm text-secondary">
          Já tem conta?{' '}
          <Link to="/auth/login" className="text-accent-primary font-medium hover:underline">
            Fazer login
          </Link>
        </div>
      </div>
